...
class Particle {
  ...
  //check particle position, check mouth position, move the particle, draw the particle
  update() {
    ...
    //check collision detection with mouse position
        ...
        for(let b = a; b < particlesArray.length; b++) {
            ...
            * (particlesArray[a].y - particlesArray[b].y));
            if(distance < (canvas.width/7) * (canvas.height/7)) {
                ...
                ctx.strokeStyle = 'rgba(65, 184, 131,'+ opactiyValue + ')';
                ctx.lineWidth = 1;
                ctx.beginPath();
